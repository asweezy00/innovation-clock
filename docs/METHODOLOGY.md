...
- **Ingredient-set filter (exact equality).** A candidate application is accepted if its active-ingredient set **exactly equals** the cohort drug's expected ingredient set, **or** the brand name matches. Exact equality (not subset) cuts both ways: a combination drug (Entresto = sacubitril + valsartan) must list *both* members, so valsartan-only products are excluded; and a single-ingredient drug (Jardiance = empagliflozin) does **not** absorb fixed-dose combinations or co-packs that merely *contain* it (Synjardy, Glyxambi, Trijardy, the Kisqali-Femara co-pack) — those products' new indications belong to them, not to the single agent. (An earlier subset rule let single agents absorb ~20 such combination-product supplements, double-counting Trijardy's 2020 supplement under both Jardiance and Tradjenta; exact matching removes them.)
...
**Documented FDA limits.** We request openFDA's maximum of 1,000 results per query. A handful of *very common* shared ingredients (e.g. metformin, tenofovir) match more products than that, but originator resolution does not depend on an exhaustive listing — each drug is also searched by its own ingredient tokens, and combination products are excluded by the exact-ingredient-set match above, so the truncation does not affect any cohort drug's anchor or event set. Efficacy supplements are a **proxy** for "new indications": they capture most new-disease approvals but also some population/line-of-therapy expansions and can miss a handful of original-label indications. This bias is consistent across modalities, so cross-modality comparisons remain meaningful.
...
- **Part D** carries per-manufacturer rows plus an `Mftr_Name == "Overall"` aggregate per SKU — we keep only the Overall rows to avoid double-counting (3,598 rows).
- **Spend = the whole negotiated brand family, not one SKU.** A CMS-negotiated "drug" is a brand *family*: its Part D "Overall" spend is split across device/strength variants (Enbrel / Enbrel Mini / Enbrel SureClick; Austedo / Austedo XR) and, where CMS bundles them, across distinct brands of the same molecule (Ozempic / Rybelsus / Wegovy; NovoLog / Fiasp). We therefore **sum** the family: an Overall row joins a drug's unit when it is single-source (brand ≠ generic, so authorized generics and commodity rows are excluded) **and** its brand equals the drug's brand or a curated CMS-bundle alias, **or** is a device/strength variant of one carrying the *same* active ingredient (so the biphasic "NovoLog Mix 70-30" and the Kisqali-Femara co-pack, which have different ingredients, are excluded). This recovers ≈$4.6B that a single-row pick understates (e.g. Ozempic+Rybelsus+Wegovy = $10.9B vs $9.2B for the Ozempic SKU alone; Enbrel family $3.0B vs $2.1B).
- **Part B has no manufacturer column** (it is HCPCS-based); a drug can span multiple HCPCS rows, so Part B spend for a drug is the **sum** of its matched rows.
...
**Canonical cohort = the 40 IRA-negotiated drugs.** Every headline number, the modality table, and Figures 1–3 are computed on these 40 only (30 small molecule + 10 biologic). The broader 64-drug union (incl. top-50 Part D spenders) is reported only as context and is never mixed into the stats. `docs/FINDINGS.md` shows the explicit computation behind each headline number, regenerated on every run.
...
- **After clock**: `years_after_launch ≥ clock_year`. **Back half of the pre-clock window**: `clock_year/2 < years_after_launch < clock_year` — strictly *before* the clock, so it is disjoint from "after clock" (no boundary double-count). Both computed on the 268 efficacy-supplement events of the 40 negotiated drugs.
- **EPIC balance check — small molecules only.** EPIC moves *only* the small-molecule selection clock (yr 7 → 11); biologics already sit at yr 11 and are **unaffected**, so they cannot be removed by EPIC. We therefore count only the **30 negotiated small molecules** that were < 11 years past first approval at their cycle's selected-drug announcement (Cycle 1: 2023-08-29; Cycle 2: 2025-01-17; Cycle 3: 2026-01-27): **19 of 30** ($67.6B). Per cycle: 5/7, 8/15, 6/8. (Public Citizen's published "5 of the first 10, 8 of the next 15" counts *all* selected drugs incl. biologics; the 5 and 8 blocked are the small molecules — consistent with our count.)

## 7. Validation / sanity checks

- Indication events dated *before* the anchor approval are dropped during extraction (reformulation/transfer artifacts), so the assembled dataset contains no pre-anchor events by construction; a defensive re-check in `build_dataset.py` confirms 0 remain.
...
5. **Xolair Part B classification is contested.** CMS published no official per-drug Part B/Part D split for Cycle 3 (its fact sheets attach HCPCS J-codes "if applicable," including for Cosentyx and Xolair). Analyst counts differ: Avalere and Cardinal Health describe four primarily-Part B drugs (Botox, Cimzia, Orencia, Entyvio), while Milliman counts Xolair as a fifth under a "majority of gross spending" definition. We label Xolair Part B with a note; spend is looked up in both parts regardless, so totals are unaffected.
